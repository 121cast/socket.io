...
function Manager (server, options) {
  ...
  server.once('listening', function () {
    self.gc = setInterval(self.garbageCollection.bind(self), 300 * 1000);
  });
...
Manager.prototype.onHandshake = function (id, data) {
	// Ignored; we no longer use handshakes for Web Sockets
...
Manager.prototype.onJoin = function (id, name) {
  ...
  if (!this.roomClients[id][name]) {
    var updatedLength = this.rooms[name].push(id);
    this.roomClients[id][name] = updatedLength - 1;
  }
};
...
Manager.prototype.onLeave = function (id, room) {
  if (this.rooms[room]) {
    var index = this.roomClients[id][room];
...
Manager.prototype.generateId = function () {
  ...
  if (crypto.randomBytes) {
    crypto.randomBytes(12).copy(rand);
  	...
  	throw "CryptoLibrary not available";
  }
